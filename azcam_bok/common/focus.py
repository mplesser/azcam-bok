...
class Focus(object):
    ...
    The focus sequence perfromed is:
    --- expose
    --- move focus
    ...
    def set_pars(
        self, exposure_time, number_exposures=7, focus_step=30, detector_shift=10
    ):
        """
    ...
    def run(
        ...
        if ExpTime < 0.001:
            azcam.AzCamWarning("do not focus with 0 exposure time")
            return
        ...
        while FocusCurrentExposure <= self.number_exposures:
            ...
            if FocusCurrentExposure > 1:
                ...
                if FocusCurrentExposure == self.number_exposures:
                    azcam.utils.log("Last exposure, double shifting")
                    azcam.api.parshift(self.detector_shift)
            ...
            # integrate
            azcam.utils.log("Integrating")
            try:
                azcam.api.integrate_exposure()
            except azcam.AzCamError:
                azcam.utils.log("Focus exposure aborted")
                azcam.api.set_focus(
                ...
                azcam.utils.log("Current focus: %.3f" % fp)
                return
        ...
        azcam.utils.log("Returning focus to starting value %.3f" % FocusStartingValue)
        if self.focus_type == "step":
            steps = -1 * nsteps
    ...
    def focus_delay(self):
        """
        Delay until focus stops moving.
        """
